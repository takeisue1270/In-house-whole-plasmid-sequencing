...
**Documentation and code packaging: ready for review. Wet-lab quantities follow the controlled ONT revisions listed below, except for the explicitly marked in-house miniaturized barcoding reaction and omission of the pooled-library AMPure XP cleanup.**

## Controlled ONT sources

- Plasmid library preparation and MinION/GridION loading: `PRB_9188_v114_revK_07Apr2026`
- PromethION compatibility-limitation evidence only: `PFC_9097_v1_revN_29Jan2025`
...
3. Replaced the ambiguous in-house RA/ADB mixture with the controlled ONT adapter-dilution and attachment procedure.
4. Added the current MinION/GridION BSA-containing priming branch.
5. Removed the in-house PromethION procedure because the controlled ONT plasmid protocol does not establish a complete SQK-RBK114/FLO-PRO114M branch.
...
The controlled ONT plasmid protocol requires an equal-volume AXP cleanup after pooling. This repository's rapid branch instead transfers 11 µL of the thoroughly mixed direct pool to the official RA/ADB attachment step. This reduces handling time by removing bead incubation, magnetic separation, ethanol washes, elution, and post-cleanup quantification, but it may change library purity, recovery, barcode balance, adapter attachment, sequencing yield, and run-to-run reproducibility.
...
## PromethION publication note

PromethION is explicitly unsupported in this release. The generic ONT PromethION loading guide is insufficient because it delegates final library composition to a relevant kit-specific protocol. No PromethION reagent ratio is inferred or published here.
